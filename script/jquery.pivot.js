...
/* https://github.com/prasad83/jquery.pivot - customized (support for bootstrap class + JSON path) */
var Jquerypivot;
    ...
    (function (_Adapter) {
        ...
        var Adapter = (function () {
            ...
            Adapter.prototype.parseJSONsource = function (data) {
				...
				function _getDataValue(row, key) {
					var value = row;
                ...
                for (rowIndex = 0, rowcount = data.rows.length; rowIndex < rowcount; rowIndex += 1) {
                    ...
                    for (i = 0; i < this.alGroupByCols.length; i += 1) {
                        groupbyValue = trim(_getDataValue(row, this.alGroupByCols[i].colvalue));
                        groupbyText = _getDataValue(row, this.alGroupByCols[i].coltext);
                        sortbyValue = trim(_getDataValue(row, this.alGroupByCols[i].sortbycol));
                    ...
                    pivotValue = trim(_getDataValue(row, this.pivotCol.colvalue));
                    pivotSortBy = trim(_getDataValue(row, this.pivotCol.sortbycol));
                    resultValues = [];
                    for (i = 0; i < this.resultCol.length; i += 1) {
                        resultValues.push(trim(_getDataValue(row, this.resultCol[i].colvalue)));
                    }
        ...
        var pivot = (function () {
            function pivot(suppliedoptions) {
                ...
                this.makeCollapsed = function (adapter, $obj) {
                    var i, j, col, $pivottable, aggVals = [], sb = new Jquerypivot.Lib.StringBuilder('<table class="pivot table table-condensed">'), gbCols = adapter.alGroupByCols, pivotCols = adapter.uniquePivotValues, results; // Prasad - Customized to bootstrap
